**MINIONS LAB — RESEARCH NOTEBOOK**

You are tasked with creating the complete initial foundation for **minions-lab** — structured experiments and hypotheses. This is part of the Minions ecosystem, a universal structured object system designed for building AI-native tools.

---

**PROJECT OVERVIEW**

`minions-lab` is a structured system for designing, running, and analyzing experiments and hypotheses. It provides the scientific method as code: formulate hypotheses, design experiments with controlled variables, run trials, collect results, and draw conclusions — all as structured minions.

The core insight: AI research is iterative and experimental by nature. Agents need infrastructure to systematically test ideas, track what worked, and learn from failures. This turns ad hoc experimentation into reproducible science.
...
1. **`hypothesis`** — A testable claim with predicted outcome
   - Fields: `statement` (textarea, required), `prediction` (textarea), `status` (select: proposed/testing/supported/refuted/inconclusive), `confidence` (number, 0-1), `domain` (string)
   - Relations: `parent_of` → experiments, `references` → evidence

2. **`experiment`** — A designed test of a hypothesis
   - Fields: `design` (textarea, required), `methodology` (textarea), `variables` (json), `controlGroup` (textarea), `sampleSize` (number), `status` (select: designed/running/completed/abandoned)
   - Relations: `references` → hypothesis, `parent_of` → trials

3. **`trial`** — A single execution of an experiment
   - Fields: `input` (json, required), `output` (json), `duration` (number), `success` (boolean), `notes` (textarea), `executedAt` (date)
   - Relations: `references` → experiment

4. **`result`** — Aggregated findings from trials
   - Fields: `summary` (textarea, required), `data` (json), `statisticalSignificance` (number), `effectSize` (number), `charts` (json)
   - Relations: `references` → experiment

5. **`conclusion`** — A derived insight from results
   - Fields: `finding` (textarea, required), `confidence` (number, 0-1), `implications` (textarea), `limitations` (textarea), `nextSteps` (textarea)
   - Relations: `references` → result, `references` → hypothesis

6. **`variable`** — An experimental variable definition
   - Fields: `name` (string, required), `variableType` (select: independent/dependent/controlled), `unit` (string), `range` (json), `description` (textarea)
   - Relations: `part_of` → experiment

---

**BEYOND STANDARD PATTERN**

- Statistical significance calculator on trial results
- `compare(exp1, exp2)` generating structured diff of variables and outcomes
- Reproducibility score based on experiment documentation completeness
- Integration with `minions-sheets` for quantitative result storage
- Citation linking via `references` relations to source notes or articles
...
```bash
lab hypothesis new "Smaller prompts reduce hallucination"
lab experiment new --hypothesis <id>
lab trial run <exp-id>
lab compare exp-1 exp-2
lab summary <hypothesis-id>
lab reproduce-score <exp-id>
```
...
Agents can systematically test and validate approaches, track experimental history, and make data-driven decisions about what strategies work.
...
Complete Python port with identical functionality. Published to PyPI as `minions-lab`.
